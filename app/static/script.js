  ...
  fetch("/api/stations")
      ...
      data.forEach((station) => {
        ...
        marker.addListener("click", () => {
          ...
          google.charts.setOnLoadCallback(() => {
            ...
            chartData.addRows([
                ['Available Bikes', station.available_bikes], // 使用实际数据
                ['Free Stands', station.available_bike_stands], // 使用实际数据
              ]);
            ...
            const options = {
                ...
                width: 300,
                height: 200,
                chartArea: { width: '70%', height: '70%' }, // 调整图表区域
                colors: ['#2ecc71', '#3498db'], // 自定义颜色
            };

            // Draw the chart
            try {
                const chart = new google.visualization.BarChart(`chart_div_${station.station_id}`);
                chart.draw(chartData, options);
              } catch (error) {
                console.error("❌ Error drawing chart:", error);
                chartElement.innerHTML = '<p style="color: red;">Failed to load chart.</p>';
              }
        });
    ...
    .catch((error) => {
      ...
      document.getElementById("loading").innerHTML =
        '<p style="color: red;">Failed to load stations. Please try again later.</p>';
    })
    .finally(() => {
